...
class ConvertMovieTweetingsToCrowdRec {
	...
	static main(args) {
		...
		movies_in.eachLine { movie ->
			...
			if ( movie_els != null && movie_els.size() >= 1 ) {
				...
				addEntity(entities_out, MOVIE_ETYPE, movieid, [title:title], [])
			}
		}
		...
		ratings_in.eachLine { rating, rowindex ->
			...
			if ( rating_els != null && rating_els.size() >= 4 ) {
				...
				def ts = rating_els[3]
				addRelation(relations_out, RATING_RTYPE, rowindex, ts, [rating:rating_value.toInteger()], [subject:createEntityRef(USER_ETYPE,userid), object:createEntityRef(MOVIE_ETYPE,movieid) ])
			}
		}
	}
	...
	static addEntity (file, etype, eid, propset, linkedentities) {
		def props = toJSON(propset)
		def links = toJSON(linkedentities)
